...
impl Questions {
    ...
    pub fn set_adhoc(&mut self, question: DbQuestion) -> bool {
        if let Some(id) = self.ques_haystack.iter().position(|x| x.id == question.id) {
            ...
            emit!(Popup(
                "not",
                vec![format!("未找到题目：{} {}", question.id, question.title)]
            ));
        };
        return false;
    }
...
impl Questions {
                ...
                .emit_if_error()
            {
                ...
                tokio::spawn(async move {
                    if let Some(selected_lang) =
                        emit!(SelectPopup("选择已有解答的语言", cloned_langs)).await
                    {
                        ...
                        if let Ok(f) = selected_sol_file.emit_if_error() {
                            if let Ok(contents) = f.read_contents().await.emit_if_error() {
                                let lang = f.language;
                                let editor = match leetcode_core::EditorDataRequest::new(
                                    f.title_slug.clone(),
                                )
                                .send()
                                .await
                                ...
                                let question_id = editor.data.question.question_id;
            ...
            tokio::spawn(async move {
                    ...
                    .emit_if_error()
                {
                    ...
                    .await
                    {
                        ...
                        let question_content = editor_data
                            .data
                            .question
                            .translated_content
                            .as_deref()
                            .filter(|s| !s.trim().is_empty())
                            .unwrap_or(&editor_data.data.question.content);
    ...
    pub fn toggle_daily_question(&self) -> bool {
        tokio::spawn(async move {
            let daily_challenge_question = match daily_coding_challenge::Query::new()
                .send()
                .await
                .emit_if_error()
            {
            ...
            let mut db_question = match DbQuestion::try_from(
            ...
            if db_question.save_to_db().emit_if_error().is_err() {
                return;
            }
            emit!(Topic(DbTopic::new("all")));
            ...
            let quests: Vec<Rc<DbQuestion>> = self
                ...
                .filter(|q| {
                    let search_string = format!(
                        "{} {} {} {}", // id, topics, translated title, English slug
                        q.id,
                        q.topics
                            .iter()

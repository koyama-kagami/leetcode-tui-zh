...
mod chinese_tests {
    use super::*;
    #[test]
    fn chinese_and_legacy_solution_filenames() {
        for (file, id) in [
            ("0001_two-sum_11.py", "1"),
            ("LCR 001_x-test_11.py", "LCR 001"),
            ("面试题 01.01_is-unique-lcci_11.py", "面试题 01.01"),
        ] {
            let sol = SolutionFile::try_from(PathBuf::from(file)).unwrap();
            assert_eq!(sol.question_id, id);
            assert_eq!(sol.language, Language::Python3);
        }
        assert!(SolutionFile::try_from(PathBuf::from("notes.txt")).is_err());
    }
}
...
impl TryFrom<PathBuf> for SolutionFileManager {
    ...
    fn try_from(value: PathBuf) -> Result<Self, Self::Error> {
        ...
        for maybe_entry in std::fs::read_dir(value)? {
            ...
            match maybe_sol_file {
                Ok(sol_file) => sfm.add_solution_file(sol_file),
                Err(
                    CoreError::FileNameDoesNotExistError(_)
                    | CoreError::FileNameFormatDoesNotMatch(_)
                    | CoreError::Utf8ValidityError(_),
                ) => continue,
...
impl TryFrom<PathBuf> for SolutionFile {
    ...
    fn try_from(value: PathBuf) -> Result<Self, Self::Error> {
        let regex = FILENAME_REGEX.get_or_init(|| {
            Regex::new(r"^(?P<q_id>[^_]+)_(?P<slug>[\w-]+)_(?P<lang_id>\d+)\.(?P<ext>\w+)$")

...
impl App {
    pub async fn run() -> Result<()> {
        ...
        while let Some(event) = app.signals.recv().await {
            match event {
                ...
                Event::Paste(text) => {
                    if app.cx.input.visible {
                        app.cx.input.paste(&text);
                        emit!(Render);
                    }
                }
                ...
                Event::Error(e) => app.dispatch_popup(Some("错误".into()), vec![e]),
        ...
        if let Some(term) = &mut self.term {
            ...
            let result = (|| -> Result<()> {
                let parts = shlex::split(editor)
                    .ok_or_else(|| color_eyre::eyre::eyre!("EDITOR 格式不正确"))?;
                let (program, args) = parts
                    .split_first()
                    .ok_or_else(|| color_eyre::eyre::eyre!("EDITOR 不能为空"))?;
                let status = std::process::Command::new(program)
                    .args(args)
                    .arg(&file_path)
                    .status()?;
                if !status.success() {
                    color_eyre::eyre::bail!("编辑器退出状态：{status}");
                }
                Ok(())
            })();
    ...
    async fn dispatch_db_update(&mut self) {
        let selected_topic = self
            ...
            .hovered()
            .cloned()
            .unwrap_or_else(|| DbTopic::new("hot-100"));
        tokio::spawn(async move {
            if let Err(error) = update_database_questions(true).await {
                emit!(Error(error.to_string()));
                return;
            }
            emit!(Topic(selected_topic));
            emit!(Render);
        });
    }

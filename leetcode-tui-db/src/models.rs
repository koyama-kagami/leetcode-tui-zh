...
impl TopicQuestionMap {
    fn new(topic_id: &str, question_id: String) -> Self {
        Self {
    ...
    pub(crate) fn save_mapping<'a>(question: &question::DbQuestion) -> DBResult<()> {
        for topic in question.get_topics() {
            let topic_question_mapping = Self::new(&topic.slug, question.id.clone());
    ...
    pub(crate) fn get_all_question_by_topic(topic: &DbTopic) -> DBResult<Vec<String>> {
...
impl QuestionTopicMap {
    fn new(question_id: String, topic_id: &str) -> Self {
        Self {
    ...
    pub(crate) fn save_mapping<'a>(question: &question::DbQuestion) -> DBResult<()> {
        for topic in question.get_topics() {
            let question_topic_mapping = Self::new(question.id.clone(), &topic.slug);
